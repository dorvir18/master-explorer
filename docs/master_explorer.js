...
(function () {
  ...
  MX.ui = (function () {
    ...
    function nudgeNext() {
      const btn = $('mx-next');
      if (!btn) return;
      // visual cue without relying on CSS
      btn.classList.add('mx-next-nudge');
      const prev = btn.style.boxShadow;
      btn.style.boxShadow = '0 0 0 4px rgba(76,175,80,.25)';
      window.setTimeout(() => {
        btn.classList.remove('mx-next-nudge');
        btn.style.boxShadow = prev || '';
      }, 900);
    }

    function clearNextCue() {
      const btn = $('mx-next');
      if (!btn) return;
      btn.classList.remove('mx-next-nudge');
      btn.style.boxShadow = '';
...
return { showToast, pulseSuccess: () => pulse('success'), pulseFail: () => pulse('fail'), nudgeNext, clearNextCue, setProgress, setMedals, setHeader, setSoundButton };
  })();
      ...
      host.querySelectorAll('.mx-choice').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          if (noWrong) {
            ...
            MX.ui.nudgeNext && MX.ui.nudgeNext();
            MX.ui.nudgeNext && MX.ui.nudgeNext();
return;
          }
          if (idx === correctIndex) {
            MX.ui.showToast('Верно!', 'success');
          host.querySelectorAll('.mx-choice').forEach(b => { b.disabled = true; });
            completeMission(mission.id);
            MX.ui.nudgeNext && MX.ui.nudgeNext();
            MX.ui.nudgeNext && MX.ui.nudgeNext();
} else {
    ...
    function render() {
      MX.ui.clearNextCue && MX.ui.clearNextCue();
      const id = MX.state.currentMissionId;
