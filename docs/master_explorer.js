...
(function () {
  ...
  MX.router = (function () {
    ...
    function renderTap(mission, variant) {
      ...
      const correctIndex = (variant && variant.answer && Number.isFinite(Number(variant.answer.correctIndex)))
        ? Number(variant.answer.correctIndex)
        : null;
      ...
      const host = mountInteractive(`
        <div class="mx-interactive">
          ...
          <div class="mx-result" style="display:none"></div>
        </div>
      `);
      ...
      let locked = false;
      const resultEl = host.querySelector('.mx-result');

      function finishSuccess(selectedIdx) {
        if (locked) return;
        locked = true;

        // Visual: mark selected + disable all
        host.querySelectorAll('.mx-choice').forEach((b) => {
          b.disabled = true;
          const i = Number(b.getAttribute('data-idx') || 0);
          if (i === selectedIdx) b.classList.add('mx-choice--selected');
          if (!noWrong && correctIndex !== null && i === correctIndex) b.classList.add('mx-choice--correct');
          if (!noWrong && correctIndex !== null && i === selectedIdx && selectedIdx !== correctIndex) b.classList.add('mx-choice--wrong');
        });

        const pickedText = (choices[selectedIdx] != null) ? String(choices[selectedIdx]) : '';
        MX.ui.showToast(noWrong ? `Выбрано: ${pickedText}` : 'Верно!', 'success');

        completeMission(mission.id);

        // Hint for manual navigation
        if (resultEl) {
          resultEl.style.display = 'block';
          resultEl.textContent = 'Готово! Нажми «Далее» →';
        }

        // Light attention on Next button
        const nextBtn = document.getElementById('mx-next');
        if (nextBtn) {
          nextBtn.classList.add('mx-next-ready');
          setTimeout(() => nextBtn.classList.remove('mx-next-ready'), 1200);
        }
      }
      ...
      host.querySelectorAll('.mx-choice').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          // No-wrong-answer missions: any choice completes
          if (noWrong || correctIndex === null) {
            finishSuccess(idx);
            return;
          }
          ...
          if (idx === correctIndex) {
            ...
            btn.classList.add('mx-choice--shake');
            setTimeout(() => btn.classList.remove('mx-choice--shake'), 350);
          }
        });
      });
    }
    ...
    function renderMulti(mission, variant) {
      ...
      const host = mountInteractive(`
        <div class="mx-interactive">
          <div class="mx-choices mx-choices--wrap">${items}</div>
          <div class="mx-row" style="margin-top:10px; display:flex; gap:10px; align-items:center;">
            <button class="mx-btn mx-btn--primary" id="mx-multi-check" type="button">Готово</button>
            <button class="mx-btn mx-btn--ghost" id="mx-multi-reset" type="button">Сброс</button>
          </div>
          <div class="mx-result" style="display:none; margin-top:10px;"></div>
        </div>
      `);
      ...
      const resetBtn = host.querySelector('#mx-multi-reset');

      function setButtonState(btn, isOn) {
        btn.setAttribute('aria-pressed', isOn ? 'true' : 'false');
        if (isOn) btn.classList.add('mx-choice--selected');
        else btn.classList.remove('mx-choice--selected');
      }
      ...
      host.querySelectorAll('.mx-choice--toggle').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          setButtonState(btn, selected.has(idx));
        });
      });
      ...
      function finishSuccess() {
        ...
        // Disable buttons
        host.querySelectorAll('.mx-choice--toggle').forEach(btn => (btn.disabled = true));
      ...
      if (resetBtn) {
        resetBtn.addEventListener('click', () => {
          if (locked) return;
          selected.clear();
          host.querySelectorAll('.mx-choice--toggle').forEach(btn => setButtonState(btn, false));
          MX.ui.showToast('Сбросили выбор.', 'step');
        });
      }
      ...
      if (checkBtn) {
        checkBtn.addEventListener('click', () => {
          ...
          // Check: all correct are selected and no extra
          let ok = true;
          ...
          if (ok) {
            ...
            host.querySelectorAll('.mx-choice--toggle').forEach(btn => {
              const idx = Number(btn.getAttribute('data-idx') || 0);
              if (correct.has(idx)) btn.classList.add('mx-choice--correct');
              if (selected.has(idx) && !correct.has(idx)) btn.classList.add('mx-choice--wrong');
            });
            finishSuccess();
          } else {
            MX.ui.showToast('Почти! Проверь ещё раз.', 'fail');
            MX.ui.pulseFail();
            MX.audio.playSound('fail');
          }
        });
    ...
    function renderCreative(mission, variant) {
      ...
      const placeholder = variant?.ui?.placeholder || 'Хочешь написать? (необязательно)';
      const host = mountInteractive(`
        <div class="mx-interactive">
          ${enableInput ? `<textarea class="mx-textarea" id="mx-creative-input" rows="3" placeholder="${escapeHtml(placeholder)}"></textarea>` : ''}
      ...
      const input = host.querySelector('#mx-creative-input');
      ...
      if (btn) {
        btn.addEventListener('click', () => {
          ...
          if (variant?.ui?.saveResponse && input) {
            MX.state.responses = MX.state.responses || {};
            MX.state.responses[String(mission.id)] = String(input.value || '').trim();
            MX.saveState();
          }
    ...
    function renderDrag(mission, variant) {
      ...
      function check() {
        ...
        if (ok) {
          ...
          completeMission(mission.id);
            const hint = host.querySelector('.mx-result') || null;
            if (hint) { hint.style.display='block'; hint.textContent='Готово! Нажми «Далее» →'; }
            const nextBtn = document.getElementById('mx-next');
            if (nextBtn) { nextBtn.classList.add('mx-next-ready'); setTimeout(()=>nextBtn.classList.remove('mx-next-ready'),1200); }
  ...
  function innerPrompt(variant) {
    if (variant && variant.ui && variant.ui.showInnerPrompt) {
      const txt = (variant.prompt || '').toString()
        .replace(/&/g,'&amp;')
        .replace(/</g,'&lt;')
        .replace(/>/g,'&gt;')
        .replace(/"/g,'&quot;')
        .replace(/'/g,'&#39;');
      return `<div class="mx-inner-prompt">${txt}</div>`;
    }
    return '';
  }
)();
